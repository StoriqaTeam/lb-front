...
class InstagramRedirect extends Component {
	...
	async getUser(accessToken){
		let instaProfile =	await fetch(`https://api.instagram.com/v1/users/self/?access_token=${accessToken}`, {
			method: 'GET'
		})
		.then(
			res => res.json(),
		...
		.then(json => {
			return json.meta && json.meta.code == 200 && json.data
		})
		if (instaProfile){
...
export default connect(mapStateToProps, mapDispatchToProps)(InstagramRedirect)
